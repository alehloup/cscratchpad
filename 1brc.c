...
_proc run(void) {
    ...
    Mmap mmap_info = mmap_open(S("C:/Users/Aleh/1brc_java/measurements10k.txt")); 
    {
        ...
        for (Long i = 1; i < len; ++i /* SKIPS THE FIRST LETTER OF THE LINE*/) {
            ...
            city_counts[city_hash] += 1;
        }
    }
    mmap_close(mmap_info);

    for (Int i = 0; i < ncity; ++i) {
        printf("%s: %lld\n", city_names[i], city_counts[hash_order[i]]);
        break;
    }
}
